...
子集实现的部分：内置函数只做了 46 个常用的（OPA 的内置函数集远大于此，还包含时间、加密、
HTTP 调用、图遍历等类别）；数字用 `Double`，不是 OPA 的任意精度十进制，超过 2^53 会丢精度；
正则改用 `moonbitlang/regexp`，语法是其子集。

暂不支持的部分：`with` 修饰符（遇到时报编译错误，而不是含糊的语法错误）、未绑定下标变量
的隐式迭代（需要显式写 `some x in data.arr`）、非字符串对象键、`walk`、点号规则头名
（`a.b := 1`）。
...
边缘环境里跑。当前的主要限制是内置函数只有 46 个、不支持 `with` 与隐式迭代、数字精度受
